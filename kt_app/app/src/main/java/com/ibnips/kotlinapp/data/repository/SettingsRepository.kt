...
class SettingsRepository @Inject constructor(
    ...
    val mockModeEnabled: Flow<Boolean> = context.dataStore.data.map { it[MOCK_MODE_ENABLED] ?: false }
    val positionUpdateFreq: Flow<Long> = context.dataStore.data.map { it[POSITION_UPDATE_FREQ] ?: 4000L }
    val serverEndpoint: Flow<String> = context.dataStore.data.map { it[SERVER_ENDPOINT] ?: "" }
